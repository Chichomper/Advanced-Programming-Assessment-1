...
# Parse jokes into a list of (setup, punchline)
jokes = [line.split('?') for line in jokes_data.strip().split('\n')]
unused_jokes = jokes.copy()

def tellJoke():
    global unused_jokes
    if not unused_jokes:  # Reset when all jokes are used
        unused_jokes = jokes.copy()
...
def main():
    ...
    while True:
        ...
        if user_input == "alexa tell me a joke":
            ...
            print("Say 'Alexa tell me a Joke' to get a joke or 'quit' to exit.")
